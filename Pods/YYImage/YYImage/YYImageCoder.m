...
CGImageRef YYCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, YYCGColorSpaceGetDeviceRGB(), bitmapInfo);//创建位图图形上下文
        if (!context) return NULL;
        CGContextDrawImage(context, CGRectMake(0, 0, width, height), imageRef); //绘制到画布上
        CGImageRef newImage = CGBitmapContextCreateImage(context);// 根据位图上下文，创建新的解压缩位图,CGImage
        CFRelease(context);
        return newImage;
